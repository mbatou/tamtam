...
export default function AdminPayoutsPage() {
  ...
  return (
    <div className="p-6 max-w-6xl">
      <h1 className="text-2xl font-bold mb-8">{t("admin.payouts.title")}</h1>

      {pendingPayouts.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-bold mb-4 text-primary-light">{t("admin.payouts.pending", { count: pendingPayouts.length })}</h2>
              ...
              <div key={payout.id} className="glass-card p-5 border-l-4 border-l-primary-light">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-white/40">
                    <span>{payout.provider === "wave" ? t("common.wave") : t("common.orangeMoney")}</span>
                    <span>·</span>
      ...
      <h2 className="text-lg font-bold mb-4">{t("admin.payouts.history")}</h2>
      <div className="glass-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase">{t("admin.payouts.echoLabel")}</th>
                <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase">{t("common.amount")}</th>
                <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase">{t("admin.payouts.via")}</th>
                <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase">{t("common.status")}</th>
                <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase">{t("common.date")}</th>
              </tr>
            </thead>
            <tbody>
              {processedPayouts.map((payout) => (
                <tr key={payout.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-4 text-white/60">{payout.provider === "wave" ? t("common.wave") : t("common.orangeMoney")}</td>
                  <td className="px-5 py-4"><span className={`badge-${payout.status}`}>{payout.status === "sent" ? t("common.sent") : t("common.failed")}</span></td>
        ...
        {processedPayouts.length === 0 && <div className="p-8 text-center"><p className="text-white/30 text-sm">{t("admin.payouts.noPayments")}</p></div>}
      </div>
    </div>
  );
}
