"use client";

import { useCallback, useEffect, useState } from "react";
import { formatFCFA } from "@/lib/utils";
import FixButton from "./FixButton";
import { fetchJson } from "./types";

interface DriftDetail {
  echo_id: string;
  name: string | null;
  stuck_fcfa: number;
}

interface DriftReport {
  echos: number;
  total_fcfa: number;
  echos_detail?: DriftDetail[];
}

interface SettleResult {
  settled_echos: number;
  settled_fcfa: number;
  failures: { echo_id: string; error: string }[];
}

/**
 * One-click settlement for stuck pending earnings (pending_balance without a
 * backing pending_earnings row — see /api/superadmin/reconcile-pending-drift).
 * The unlock cron settles only from pending_earnings, so this money would
 * otherwise never reach available_balance and never be withdrawable.
 */
export default function StuckPendingCard() {
  const [report, setReport] = useState<DriftReport | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showDetail, setShowDetail] = useState(false);

  const load = useCallback(() => {
    setLoading(true);
    setError(null);
    fetchJson<DriftReport>("/api/superadmin/reconcile-pending-drift")
      .then(setReport)
      .catch((err) => setError(err instanceof Error ? err.message : "Erreur réseau"))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => load(), [load]);

  const settle = async () => {
    const data = await fetchJson<SettleResult>("/api/superadmin/reconcile-pending-drift", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dry_run: false }),
    });
    const failures = data.failures.length > 0 ? ` · ${data.failures.length} échec(s)` : "";
    return `Régularisé : ${formatFCFA(data.settled_fcfa)} pour ${data.settled_echos} échos${failures}`;
  };

  if (loading && !report) {
    return (
      <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 text-xs text-gray-500">
        Vérification des gains en attente...
      </div>
    );
  }

  const clean = report !== null && report.total_fcfa <= 0;

  return (
    <div
      className={`rounded-xl border p-4 ${
        clean ? "bg-green-500/[0.06] border-green-500/20" : "bg-yellow-500/[0.06] border-yellow-500/25"
      }`}
    >
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <p className="text-sm font-bold text-white">Gains en attente non débloquables</p>
          {clean ? (
            <p className="text-xs text-green-400 mt-1">
              ✓ Aucun gain bloqué — tout est couvert par pending_earnings
            </p>
          ) : (
            <p className="text-xs text-yellow-400 mt-1">
              {report?.echos} échos ne pourront jamais toucher {formatFCFA(report?.total_fcfa || 0)} :
              leur solde en attente n&apos;a pas de ligne pending_earnings, donc le cron ne le
              débloquera jamais.
            </p>
          )}
          {error && <p className="text-xs text-red-400 mt-1">{error}</p>}
        </div>
        <div className="flex items-start gap-2">
          {!clean && (report?.echos_detail?.length || 0) > 0 && (
            <button
              onClick={() => setShowDetail((v) => !v)}
              className="text-xs text-gray-500 hover:text-gray-300 bg-white/5 px-3 py-1.5 rounded-lg"
            >
              {showDetail ? "Masquer" : "Détail"}
            </button>
          )}
          {!clean && report && (
            <FixButton
              label="Débloquer"
              amountFcfa={report.total_fcfa}
              beneficiary={`${report.echos} écho${report.echos > 1 ? "s" : ""}`}
              detail={
                "L'argent passe de « en attente » à « disponible » (retirable). " +
                "Opération idempotente et auditée."
              }
              onConfirm={settle}
              onDone={load}
            />
          )}
        </div>
      </div>

      {showDetail && !clean && (
        <div className="mt-3 pt-3 border-t border-white/[0.06] max-h-48 overflow-y-auto">
          {report?.echos_detail?.map((d) => (
            <div
              key={d.echo_id}
              className="flex items-center justify-between py-1 text-xs border-b border-white/[0.03] last:border-0"
            >
              <span className="text-white/60">{d.name || d.echo_id.slice(0, 8)}</span>
              <span className="text-yellow-400 font-medium">{formatFCFA(d.stuck_fcfa)}</span>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
