...
export default function SuperadminGamification() {
  ...
  const activityFeed: { type: "achievement" | "streak"; date: string; echoId: string; amount: number; detail?: string }[] = [];
  for (const a of achievements) {
    activityFeed.push({ type: "achievement", date: a.achieved_at, echoId: a.echo_id, amount: a.reward_fcfa });
  }
  for (const s of streakRewards) {
    activityFeed.push({ type: "streak", date: s.credited_at, echoId: s.echo_id, amount: s.reward_fcfa });
  }
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-white/30 font-mono truncate">
                      {item.echoId.substring(0, 8)}… · {new Date(item.date).toLocaleDateString("fr-FR")}
                    </p>
                  </div>
                </div>
