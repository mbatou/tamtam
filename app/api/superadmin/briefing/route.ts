...
export async function GET() {
  ...
  const [
    ...
    supabase.from("users").select("*", { count: "exact", head: true }).eq("role", "echo").is("deleted_at", null).gte("created_at", todayStart),
    supabase.from("clicks").select("*", { count: "exact", head: true }).gte("created_at", todayStart),
    supabase.from("users").select("*", { count: "exact", head: true }).eq("role", "echo").is("deleted_at", null).gte("created_at", yesterdayStart).lt("created_at", todayStart),
