...
import { alertLeadReceived } from "@/lib/notifications/ops-alerts";
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await rateLimit(`lead:${ip}`, 3, 3600000);
  if (!allowed) {
  ...
  try {
    await alertLeadReceived({ business_name, contact_name, email, whatsapp, message });
  } catch (e) {
