...
export default function RythmesPage() {
  ...
  const tabs = [
    { key: "available" as const, label: "Disponibles", count: availableCampaigns.length },
    { key: "mine" as const, label: "Mes Rythmes", count: myActiveLinks.length },
    { key: "done" as const, label: "Terminés", count: finishedLinks.length },
  ];
  ...
  return (
    <div className="px-4 py-5">
      ...
      <div className="bg-gradient-to-r from-yellow-400/10 to-yellow-600/10 border border-yellow-400/20 rounded-xl p-3 mb-4 flex items-center gap-3">
        <span className="text-xl">👻</span>
        <div>
          <span className="text-yellow-300 font-bold text-xs">{t("snapchat.arrivingNextWeek")}</span>
          <span className="text-gray-400 text-[10px] block">{t("snapchat.doubleYourEarnings")}</span>
        </div>
      </div>
                ...
                <div className="p-4">
                  ...
                  {(campaign.objective || "traffic") === "awareness" && (
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-2 text-xs text-blue-300 mb-2">
                      Campagne Notori&eacute;t&eacute; &mdash; Partage le visuel + lien ensemble
                    </div>
                  )}
        ...
        <div className="space-y-3">
          {finishedLinks.length === 0 ? (
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-6 text-center">
              ...
              <p className="text-sm font-semibold mb-1">Aucun rythme termin&eacute;</p>
              <p className="text-xs text-white/30">Tes campagnes termin&eacute;es apparaitront ici</p>
            </div>
          ) : (
              ...
              return (
                <div key={link.id} className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-white/40">{link.click_count} clics</span>
      ...
      {campaigns.length === 0 && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          ...
          >
            ...
            <span className="text-[10px] text-[#D35400] block mt-0.5">+150 FCFA</span>
          </button>
          <button
