    ...
    <>
      ...
      <div className="bg-gradient-to-r from-green-900/50 via-yellow-900/30 to-red-900/50 border border-yellow-500/30 rounded-xl p-4 mb-4">
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between text-sm mb-1">
            ...
            <span className="text-yellow-400 text-xs">
              Prochain œuf dans {clicksToNext} clics
            </span>
          </div>
        ...
        <div className="flex gap-4 mt-3 text-sm">
          <span className="text-green-400">🥚 {participation.eggs_earned} œufs craqués</span>
          <span className="text-yellow-400">💰 {participation.total_won} FCFA gagnés</span>
        </div>
      </div>
            ...
            {recentCracks.slice(0, 5).map((crack) => (
              <div key={crack.id} className="flex items-center justify-between text-sm">
                <span className="text-gray-300 text-xs">
                  {crack.users?.name || "Écho"} a craqué un œuf {crack.tier} !
                </span>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
          {cracking && !reward && (
            <div className="text-center">
              ...
              <p className="text-white text-xl mt-4 animate-pulse">Ça craque...</p>
            </div>
          )}
          {reward && (
