...
export const createCampaignSchema = z.object({
  ...
  cpa_amount: z.coerce.number().int().min(100, "CPA minimum 100 FCFA").max(1000000).optional().nullable(),
...
export const updateCampaignSchema = z.object({
  ...
  pixel_id: z.string().max(50).optional().nullable(),
  pricing_model: z.enum(["cpc", "cpa"]).optional(),
  cpa_amount: z.coerce.number().int().min(100).max(1000000).optional().nullable(),
...
export const createLeadCampaignSchema = z.object({
  ...
  // Template selection
  template: z.enum(["simple", "product", "event", "app", "contact"]).optional(),
